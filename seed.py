...
Создаёт:
    ...
    2 лендинга      (mis-demo, tender-consult)
...
def seed(reset=False):
    ...
    with app.app_context():
        ...
        LANDINGS = [
            {
                'name':        'Запишитесь на демо МИС для вашего учреждения',
                'slug':        'mis-demo',
                'campaign':    campaigns[0],   # Email-рассылка (active)
            },
            {
                'name':        'Получите консультацию по участию в тендерах 44-ФЗ',
                'slug':        'tender-consult',
                'campaign':    campaigns[2],   # Тендеры (active)
            },
        ]
        for lp_data in LANDINGS:
            existing = LandingPage.query.filter_by(slug=lp_data['slug']).first()
            if existing:
                print(f"  ~ /{lp_data['slug']} уже существует, пропускаем")
                continue
            lp = LandingPage(
                name=lp_data['name'],
                slug=lp_data['slug'],
                campaign_id=lp_data['campaign'].id,
                is_active=True,
            )
            db.session.add(lp)
            print(f"  + /landing/{lp_data['slug']}  ->  \"{lp_data['campaign'].name}\"")
        db.session.flush()
